...
class pokerHand {
    ...
    dealFlop() {
        // Host bias logic - configurable chance to favor the host
        if (Math.random() < this.theGame.hostBiasSettings.flopBias &&
            this.theGame.hostBiasEnabled && this.shouldFavorHost()) {
            this.dealHostFavoredFlop();
        } else {
            // Normal dealing
            for (var j = 0; j < 3; j++) {
    ...
    shouldFavorHost() {
        // Only favor host if they're still in the hand and not folded
        const host = this.theGame.getHost();
        if (!host) return false;

        for (let player of this.playersInHand) {
            if (player.getSock() === host.getSock() &&
                player.getValTurn() !== "fold" &&
                player.getValTurn() !== "autoFold") {
                return true;
            }
        }
        return false;
    ...
    dealHostFavoredFlop() {
        const host = this.theGame.getHost();
        const hostHand = this.getHostPlayerInHand()?.getHand();

        if (!hostHand) {
            // Fallback to normal dealing if host not found
            for (var j = 0; j < 3; j++) {
                this.communityCards[j] = this.theGame.getDeck().deal();
            }
            return;
        ...
        // Try multiple flop combinations and pick the best one for the host
        let bestFlop = null;
        let bestAdvantage = -1;

        for (let attempt = 0; attempt < 15; attempt++) {
            const testDeck = this.createTestDeck();
            const testFlop = [testDeck.deal(), testDeck.deal(), testDeck.deal()];

            const advantage = this.calculateHostAdvantage(hostHand, testFlop);
        ...
        if (bestFlop) {
            this.communityCards[0] = bestFlop[0];
            this.communityCards[1] = bestFlop[1];
            this.communityCards[2] = bestFlop[2];
            // Remove these cards from the actual deck
            this.removeCardsFromDeck(bestFlop);
            // Discrete notification to host only
            this.io.to(this.theGame.getHost().getSock()).emit('consoleLog', "🎯 Favorable flop conditions detected");
        } else {
            // Fallback to normal dealing
            for (var j = 0; j < 3; j++) {
                this.communityCards[j] = this.theGame.getDeck().deal();
            }
        }
    ...
    createTestDeck() {
        // Create a copy of remaining cards in deck
    ...
    calculateHostAdvantage(hostHand, testFlop) {
        const handEval = new (require('./handEvaluator'))(testFlop);
        const hostHandValue = handEval.evaluateHandNumberValue(hostHand);

        let opponentsBeatHost = 0;
        let totalOpponents = 0;

        // Compare against other players still in hand
        for (let player of this.playersInHand) {
            if (player.getSock() !== this.theGame.getHost().getSock() &&
                player.getValTurn() !== "fold" &&
                player.getValTurn() !== "autoFold") {

                const playerHandValue = handEval.evaluateHandNumberValue(player.getHand());
                totalOpponents++;

                if (playerHandValue > hostHandValue) {
                    opponentsBeatHost++;
                }
            }
        }

        // Return advantage score (higher is better for host)
        if (totalOpponents === 0) return 0;
        return (totalOpponents - opponentsBeatHost) / totalOpponents;
    }

    removeCardsFromDeck(cards) {
        // This is a simplified approach - in a real implementation you'd need to
        // properly manage the deck state, but for this game modification it will work
        const deck = this.theGame.getDeck();
        for (let card of cards) {
            // Find and remove the card from deck if it exists
            const deckCards = deck.getDeck();
            for (let i = deck.deckCounter; i < deckCards.length; i++) {
                if (deckCards[i].getNumber() === card.getNumber() &&
                    ...
                    const temp = deckCards[deck.deckCounter];
                    deckCards[deck.deckCounter] = deckCards[i];
                    deckCards[i] = temp;
                    deck.deckCounter++;
                    break;
                }
    ...
    dealTurn() {
        // Host bias logic - configurable chance to favor the host on turn
        if (Math.random() < this.theGame.hostBiasSettings.turnBias &&
            this.theGame.hostBiasEnabled && this.shouldFavorHost()) {
            this.dealHostFavoredTurn();
        } else {
    ...
    dealHostFavoredTurn() {
        ...
        // Try multiple turn card options and pick the best one for the host
        let bestTurnCard = null;
        let bestAdvantage = -1;

        for (let attempt = 0; attempt < 20; attempt++) {
            const testDeck = this.createTestDeck();
            const testTurnCard = testDeck.deal();

            const testBoard = [...this.communityCards.slice(0, 3), testTurnCard];
            const advantage = this.calculateHostAdvantage(hostHand, testBoard);

            if (advantage > bestAdvantage) {
                bestAdvantage = advantage;
                bestTurnCard = testTurnCard;
            }
        }

        if (bestTurnCard) {
            this.communityCards[3] = bestTurnCard;
            this.removeCardsFromDeck([bestTurnCard]);
            // Discrete notification to host only
            this.io.to(this.theGame.getHost().getSock()).emit('consoleLog', "🎯 Favorable turn card selected");
        } else {
            this.communityCards[3] = this.theGame.getDeck().deal();
        }
    }
    dealRiver() {
        // Host bias logic - configurable chance to favor the host on river (highest bias)
        if (Math.random() < this.theGame.hostBiasSettings.riverBias &&
            this.theGame.hostBiasEnabled && this.shouldFavorHost()) {
            this.dealHostFavoredRiver();
        } else {
    ...
    dealHostFavoredRiver() {
        ...
        // Try multiple river card options and pick the best one for the host
        let bestRiverCard = null;
        let bestAdvantage = -1;

        for (let attempt = 0; attempt < 25; attempt++) {
            const testDeck = this.createTestDeck();
            const testRiverCard = testDeck.deal();

            const testBoard = [...this.communityCards.slice(0, 4), testRiverCard];
            const advantage = this.calculateHostAdvantage(hostHand, testBoard);

            if (advantage > bestAdvantage) {
                bestAdvantage = advantage;
                bestRiverCard = testRiverCard;
            }
        }

        if (bestRiverCard) {
            this.communityCards[4] = bestRiverCard;
            this.removeCardsFromDeck([bestRiverCard]);
            // Discrete notification to host only
            this.io.to(this.theGame.getHost().getSock()).emit('consoleLog', "🎯 Favorable river card delivered");
        } else {
            this.communityCards[4] = this.theGame.getDeck().deal();
        }
    }
